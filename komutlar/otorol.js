...
exports.run = async(client, message, args) => {
 ...
 let rol = message.mentions.roles.first() || message.guild.roles.get(args.join(' '))
  let newRole;
  let tworole;
  if (!rol) return message.channel.send('<a:onayla:764222258881101845> Otorol ayarlamanız için bir rol etiketlemeniz gerek. `e!otorol ayarla @Üye #kanal`')
  ...
  if (!otorolkanal) return message.channel.send(':no_entry: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `e!otorol ayarla @Üye #kanal`')
  ...
  if (bymayfe == 'kapat' || bymayfe == 'sıfırla') {
    ...
    message.channel.send(`<a:bakm:764219872384647208> **Otorolü başarıyla kapattım.**`)
  }
};
